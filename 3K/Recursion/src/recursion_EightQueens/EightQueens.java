package recursion_EightQueens;//package eightqueens;
...
class EightQueen {
    ...
    public void recursiveSolve(int row) {
        ...
        if (row == 8) {
            ...
            for (int r = row; r < chessBoard.length; r++) {
    ...
    public boolean isValid(int row, int column) {
        ...
        for (int r = 0; r < chessBoard.length; r++) {
            for (int c = 0; c < chessBoard[r].length; c++) {
                if (chessBoard[r][c]) { // is queen
                    if (r == row || c == column || (Math.abs(r - row) == Math.abs(c - column)))
                        return false;
                }
            }
        }
