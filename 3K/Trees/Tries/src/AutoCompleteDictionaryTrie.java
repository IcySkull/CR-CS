...
public class AutoCompleteDictionaryTrie implements Dictionary, AutoComplete {
	...
	public boolean hasChild(TrieNode curr, char c) {
		return curr.getChild(c) != null;
	}
	...
	// 	return getWords(curr, completions, numCompletions);
	// }

	public List<String> getWords(TrieNode node, List<String> words, Integer limit) {
		if (node == null)
			return words;
		else if (limit == words.size())
			return words;
		
		if (node.endsWord())
			words.add(node.getText());
		for (Character c : node.getValidNextCharacters()) {
			getWords(node.getChild(c), words, limit);
		}
		return words;
	}
